...
function setData($columnsName = null,$id,$val=0){
    global $wpdb;
    $query = $wpdb->prepare("UPDATE prok_table SET $columnsName=%d where ID=%d",
        [$val,$id]);
    $wpdb->query( $query);
}
...
function updateData($id,$index_url,$prok_begin_index,$prok_end_index,$prok_begin,$prok_end,$name,$title,$ingr_pr,$step_pr,$autoupdate,$count_add_post,$first_number,$cat,$status,$cal,$time_cook,$serves,$statust){
    global $wpdb;
    $query = $wpdb->prepare('UPDATE prok_table SET index_url=%s,prok_begin_index=%s,prok_end_index=%s,prok_begin=%s,prok_end=%s,name=%s,title_preg=%s,ingr_pr=%s,step_pr=%s,autoupdate=%s,count_add_post=%s,first_number=%s,category=%d,status=%s,cal=%s,time_cook=%s,serves=%s,status_translate=%s  where ID=%d',
        [$index_url,$prok_begin_index,$prok_end_index,$prok_begin,$prok_end,$name,$title,$ingr_pr,$step_pr,$autoupdate,$count_add_post,$first_number,$cat,$status,$cal,$time_cook,$serves,$statust,$id]);
    $wpdb->query( $query);
}
